...
import { Chapter, GetStoryBySlugResponse, IComment, Story } from "@/types";
...
export default function StoryDetailPage() {
  ...
  const getStoryBySlug = useCallback(async () => {
    try {
      const response = await axiosInstance.get<GetStoryBySlugResponse<Story>>(
        `/api/story/${storyId}`
      );
      const { data } = response;
      if (data?.message === "Success") {
        console.log("data.data", data.data);
        handleSetStateField("storyDetail", data.data);
  ...
  const handleSetStateField = useCallback(
    (field: keyof IState, value: string | null | Story) => {
  ...
  const breadcrumbItems = [
    ...
    { title: `${state.storyDetail?.title}`, link: "#" },
  ];
  return (
    <PageContainer>
