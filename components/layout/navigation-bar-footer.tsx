...
export default function NavigationBarFooter() {
  ...
  const handleChange = useCallback((newChapterId: string) => {
    if (Array.isArray(slug)) {
      const updatedSlug = [slug[0], newChapterId];
      router.push(`/${updatedSlug.join('/')}`);
    }
  }, []);
  const currentIndex = listChapter.findIndex(chapter => chapter.id.toString() === currentChapterId);
    ...
    >
      <div className="mx-auto flex w-full max-w-1366 justify-between">
        ...
        <div className="flex items-center space-x-2">
          <Button variant="ghost" size="icon" className="hover:bg-transparent" onClick={handlePrev} disabled={currentIndex <= 0}>
            <span className="mr-1 border border-dashed border-gray-400">
              <Image
          ...
          <Button variant="ghost" size="icon" className="hover:bg-transparent" onClick={handleNext} disabled={currentIndex >= listChapter.length - 1}>
            <span className="mr-1 border border-dashed border-gray-400">
              <Image
